...
3. 标题取 `title`（缺省文件名）、描述取 `articleSummary`（缺省截取正文）、首图取正文第一个图片；
4. 标签、分类不存在**自动创建**并关联；专栏**只匹配已存在的**（含二级），不创建；
5. `createTime` / `updateTime` 按 `YYYY-MM-DD HH:mm:ss` 写入；
6. 同名文档按 `conflict_policy` 处理（SKIP 跳过 / RENAME 自动 `(n)` 重命名）。
