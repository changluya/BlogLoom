...
# Author: changlu
#
# This script is used to deploy BlogLoom to my server, apply my changes
...
project_local_path=~/work/idea-project/BlogLoom
dev_branch=master
prod_branch=prod
server=aliyun
...
function menu()
{
  echo -e "\033[35m(1)blog-backend\033[0m"
  echo -e "\033[35m(2)blog-cms-ui\033[0m"
  echo -e "\033[35m(3)blog-view-ui\033[0m"
  
  read -p "choice a module to deploy: " module
  case $module in
    1)
      echo_msg 0 "deploy blog-backend"
      module_name=blog-backend
      server_path=$api_path
      deploy_springboot
      ;;
    2)
      echo_msg 0 "deploy blog-cms-ui"
      module_name=blog-cms-ui
      server_path=$cms_path
      deploy_vue
      ;;
    3)
      echo_msg 0 "deploy blog-view-ui"
      module_name=blog-view-ui
...
function deploy_springboot()
{
  ...
  scp target/blog-backend-0.0.1.jar ${server}:${server_path}
